...
async def sci_tech_explain():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        content = await get_page_content("https://indianexpress.com/about/explained-sci-tech/", browser, 20000)
        articles = await get_article_details(content)
        return articles

if __name__ == "__main__":
    import asyncio
    print(asyncio.run(sci_tech_explain()))
